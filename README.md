...
- Due to the fact that the buttons are connected with ADC (there is a different resistor next to each button), I had to separate the menu structure into two threads, one starts the menu after pressing the button only once, the other executes the entire instructions in a loop.
